...
export const AssemblingStagesPage = () => {
  const { user } = useAuth();
  const { items, create, update, remove, refetch } = useApiResource<any>('/admin/assembly-stages');
  ...
  const countFn = (id: string) =>
    products.filter((p) => (p.assemblyStages || []).some((as: any) => (typeof as === 'string' ? as : as?._id) === id)).length;
