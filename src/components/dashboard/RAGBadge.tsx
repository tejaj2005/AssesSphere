...
export const RAGBadge = ({ status, label }: { status: RAGStatus; label?: string }) => {
  const map = {
    GREEN: { cls: 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20', icon: CheckCircle2, txt: label ?? 'Green' },
    AMBER: { cls: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20', icon: AlertTriangle, txt: label ?? 'Amber' },
    RED:   { cls: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20', icon: XCircle, txt: label ?? 'Red' },
  }[status];
  const Icon = map.icon;
  return (
