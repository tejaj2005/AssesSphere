      ...
      <div className={cn('flex h-16 items-center border-b border-white/5 shrink-0 transition-all', collapsed ? 'px-3 justify-center' : 'px-4 gap-3')}>
        {!collapsed ? (
          ...
          <Tooltip content="Expand sidebar" side="right">
            <button onClick={onToggle} className="h-10 w-10 inline-flex items-center justify-center rounded-md hover:bg-white/10 transition-colors group" aria-label="Expand">
                ...
                const link = (
                  <NavLink to={item.to} end={item.to.endsWith('/dashboard') || item.to === '/admin' || item.to.endsWith('/management')}
                    className={({ isActive }) => cn(
                      ...
                      isActive ? 'bg-[#f5af12] text-[#0a3d4d] font-semibold' : 'text-white/65 hover:bg-white/8 hover:text-white'
                    )}>
                    {({ isActive }) => (
                      <>
                        {isActive && !collapsed && (
                          <motion.span layoutId={`pill-${moduleName}`} className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-r bg-[#0a3d4d]" transition={{ type: 'spring', damping: 30, stiffness: 350 }} />
                        )}
