...
export const ModuleSidebar = ({ collapsed, onToggle, groups, moduleName, profileLink }: ModuleSidebarProps) => {
  const { user } = useAuth();
  return (
    <motion.aside
    ...
    >
        ...
        {!collapsed ? (
          <>
            ...
            <motion.div variants={sidebarText} animate="expanded" initial="expanded" className="flex-1 min-w-0">
              ...
              <p className="text-[9px] text-amber-400/80 uppercase tracking-[0.18em] font-semibold leading-tight mt-0.5 truncate">{moduleName}</p>
            </motion.div>
            <Tooltip content="Collapse sidebar" side="right">
              <button onClick={onToggle} className="h-8 w-8 inline-flex items-center justify-center rounded-md text-[#7B8CA6] hover:text-white hover:bg-[#131C2E] transition-colors shrink-0" aria-label="Collapse">
          ...
          <Tooltip content="Expand sidebar" side="right">
            <button onClick={onToggle} className="h-10 w-10 inline-flex items-center justify-center rounded-md hover:bg-[#131C2E] transition-colors group" aria-label="Expand">
        ...
        {groups.map((group, gi) => (
          <div key={gi} className={cn('mb-4', collapsed ? 'px-2' : 'px-3')}>
            {group.label && !collapsed && (
              <p className="px-3 mt-3 mb-2 text-[10px] font-semibold uppercase tracking-[0.08em] text-[#7B8CA6]/70 truncate">{group.label}</p>
            )}
            {group.label && collapsed && gi > 0 && <div className="my-3 h-px bg-[#1E2D45] mx-2" />}
            <div className="space-y-0.5">
              {group.items.map((item) => {
                const link = (
                  <NavLink to={item.to} end={item.to.endsWith('/dashboard') || item.to === '/admin' || item.to.endsWith('/management')}
                    className={({ isActive }) => cn(
                      ...
                      isActive ? 'bg-[#1A2744] text-white' : 'text-[#7B8CA6] hover:bg-[#131C2E] hover:text-[#B8C5D6]'
                    )}>
                    {({ isActive }) => (
                      <>
                        {isActive && !collapsed && (
                          <motion.span layoutId={`pill-${moduleName}`} className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-r bg-[#3B82F6]" transition={{ type: 'spring', damping: 30, stiffness: 350 }} />
                        )}
                        {isActive && collapsed && (
                          <motion.span layoutId={`pill-${moduleName}`} className="absolute left-0 top-2 bottom-2 w-[3px] rounded-r bg-[#3B82F6]" />
                        )}
                        <item.icon className="h-[18px] w-[18px] shrink-0" />
                        <AnimatePresence>
                          {!collapsed && (
                            <motion.span variants={sidebarText} initial="collapsed" animate="expanded" exit="collapsed" className="truncate">{item.label}</motion.span>
                          )}
                        </AnimatePresence>
                      </>
                    )}
      ...
      <div className={cn('border-t border-[#1E2D45] shrink-0', collapsed ? 'p-2' : 'p-3')}>
        <NavLink to={profileLink || '/admin/profile'} className={cn('group flex items-center rounded-lg transition-colors', collapsed ? 'h-11 w-11 justify-center mx-auto' : 'gap-3 px-2 py-2 hover:bg-[#131C2E]')}>
